...
#check model convergence via trace/density plots
plot(model.traits1)
require(bayestestR)
...
# Filter for South Carolina (Webb in data frame)
...
#plot for impact of human harvest on distance to access in South Carolina
require(ggplot2)
p1<-ggplot(df_summary1, aes(x = BLUP_raw, y = mean)) +
  ...
  scale_x_continuous(breaks = c(500,750,1000,1250,1500))+
  ggtitle(label ="A.",
...
predictions_df$BLUP_raw <-
  predictions_df$blups_FR*744+825  #back transfrom
...
#plot for the effect of distance to public access (risk-taking) on survival from predation in South Carolina
require(ggplot2)
...
predictions_df$BLUP_raw <-
  predictions_df$blups_FR*98.1+198  #back tranform
...
#plot markov chains and density plots
plot(model.traits)
require(bayestestR)
#compute median, 95% CrI, and Rhat values
describe_posterior(model.traits3)
...
plot(model.traits4)
require(bayestestR)
#compute 95%CrI, median, and Rhat values
...
####South Carolina human distance to edge + survival----
plot(harv_ga_da)
...
predictions_df$BLUP_raw <-
  predictions_df$blups_FR*122+133  #back transform
...
p33<-ggplot(df_summary1, aes(x = BLUP_raw, y = mean)) +
  ...
  geom_point(aes(x=BLUP_raw, y= Survival), color="#333300",alpha=0.6,size=8)+ theme_turkey()+
...
####South Carolina predators distance to access + survival----
plot(pred_ga_da)
...
#final survival plot for South Carolina
